...
- **Streaming JSON parsing** handles 5GB+ files in <200MB RAM
...
$ raps translate batch-start *.rvt --format svf2 --wait
Processing 12 models (28GB total)...
✅ All translations completed in 24m15s

# Memory usage stays predictable
$ raps translate status --all --format json | jq '.[] | select(.status == "failed")'
