...
raps admin user add-to-all-projects "new.hire@company.com" --account "$ACCOUNT_ID"

# Remove departing employee from all projects
raps admin user remove "former@company.com" --account "$ACCOUNT_ID"
```
...
```bash
raps admin user add "new.bim.manager@company.com" \
  --account "$ACCOUNT_ID" \
  --role "Project Admin" \
  --filter "name:^Active-" \
...
```bash
raps admin user add "user@company.com" \
  --account "$ACCOUNT_ID" --role "Viewer" --dry-run
...
# Only 2024 projects
raps admin user add "user@company.com" \
  --account "$ACCOUNT_ID" --role "Viewer" --filter "name:^2024-"

# Multiple patterns
raps admin user add "user@company.com" \
  --account "$ACCOUNT_ID" --role "Viewer" --filter "name:Downtown|Hospital|University"
```
...
| Command | Description |
|---------|-------------|
| `raps admin user add` | Add user to filtered projects with a role |
| `raps admin user add-to-all-projects` | Add user to all active projects |
...
raps admin user add "$EMAIL" \
  --account "$ACCOUNT_ID" --role "Viewer" --filter "$PROJECT_FILTER"

# 2. Grant edit access to Project Files
raps admin folder rights "$EMAIL" \
  --account "$ACCOUNT_ID" --permission edit --folder project_files \
  --filter "$PROJECT_FILTER"

echo "Contractor onboarded to $(raps admin project list --account "$ACCOUNT_ID" --filter "$PROJECT_FILTER" | wc -l) projects"
```
  ...
  offboard:
    ...
    steps:
      ...
      - name: Remove user from all projects
        ...
        run: |
          raps admin user remove "${{ github.event.inputs.email }}" \
