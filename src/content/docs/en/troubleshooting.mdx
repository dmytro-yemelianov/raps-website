...
## Batch upload resume (v4.5)

- **State file location** — batch upload state is saved in `~/.config/raps/batch-state.json` (Linux/macOS) or `%APPDATA%\raps\batch-state.json` (Windows).
- **Stale state** — if `--resume` fails with "no matching state", delete the state file manually and start fresh.
- **Partial failures** — when some files fail in a batch, state is saved automatically. Re-run with `--resume` to retry only failed files.
- **Different bucket** — resume only works for the same bucket and file set. Changing the target bucket requires a fresh upload.
