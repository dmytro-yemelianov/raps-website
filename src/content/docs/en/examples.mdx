...
![RAPS end-to-end workflow demo](/terminal/end-to-end-demo.gif)
...
## Custom API Calls (v4.5)

Call any APS endpoint directly using the `api` command.

```bash
# Get current user profile
raps api get /userprofile/v1/users/@me

# Create a resource with inline JSON
raps api post /oss/v2/buckets -d '{"bucketKey":"test","policyKey":"transient"}'

# Call an undocumented endpoint with custom headers
raps api get /custom/endpoint -H "X-Custom:value" --verbose
```

## Portfolio Reports (v4.5)

Generate aggregate reports across all projects in an account.

```bash
# RFI summary across all active projects
raps report rfi-summary --account "acc-123" --filter "status:active"

# Issues since last month
...
Manage ACC project templates programmatically.
