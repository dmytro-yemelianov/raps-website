...
| `raps auth inspect` | Inspect token details (scopes, expiry) |
...
Open `rapscli.xyz/device` on any device (phone, laptop, another machine), enter the short code, and authorize with your Autodesk account. The CLI polls automatically and completes login once you approve. No local browser required — works in SSH sessions, containers, and MCP servers.
