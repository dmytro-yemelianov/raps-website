...
<div class="not-prose my-8">
  ...
  <div class="flex justify-center mt-4">
    <div class="text-xs text-slate-500">Select Projects → Configure Operation → Execute → Track Progress</div>
  </div>
</div>
...
raps admin user add "$EMAIL" --account "$ACCOUNT_ID" --role "Project Admin"

# Add user to specific projects (name filter)
raps admin user add "$EMAIL" --account "$ACCOUNT_ID" --role "Project Admin" \
  --filter "name:*Downtown*"

# Preview changes with dry-run
raps admin user add "$EMAIL" --account "$ACCOUNT_ID" --role "Project Admin" --dry-run
```
...
# Dry-run to preview
raps admin user remove "departed@company.com" --account "$ACCOUNT_ID" --dry-run
```
...
raps admin user update "user@company.com" --account "$ACCOUNT_ID" --role "Project Admin"

# Update role in filtered projects
raps admin user update "user@company.com" --account "$ACCOUNT_ID" \
  --role "Project Viewer" --filter "name:Archive-*"
```

### Manage Folder Permissions

```bash
# Grant edit access to Project Files folder
raps admin folder set-permissions "contractor@partner.com" --account "$ACCOUNT_ID" \
  --level view-download-upload-edit --folder project-files

# Grant view access to Plans folder
raps admin folder set-permissions "reviewer@client.com" --account "$ACCOUNT_ID" \
  --level view-download --folder plans

# Custom folder by ID
raps admin folder set-permissions "user@company.com" --account "$ACCOUNT_ID" \
  --level folder-control --folder "urn:adsk.wipprod:fs.folder:abc123"
```
...
raps admin project list --account "$ACCOUNT_ID" --filter "name:*Downtown*"

# Filter by status and platform
raps admin project list --account "$ACCOUNT_ID" --status active --platform acc

# Limit results
raps admin project list --account "$ACCOUNT_ID" --limit 10 --output json
```
...
raps admin company search "Structural" --account "$ACCOUNT_ID"

# Create a new company
raps admin company create "Structural Design Co" \
  --trade "Structural Engineering" --city Chicago --country "United States"

# Get company details
raps admin company get "company-uuid" --account "$ACCOUNT_ID"

# Update company details
raps admin company update "company-uuid" --name "Structural Design Corp"

# Assign a user to the company
raps admin user update-account "jane@company.com" --company "company-uuid"
```

### Invite and Set Up a New User

```bash
# Create (invite) a user at the account level
raps admin user create "newhire@company.com" \
  --first-name Alex --last-name Johnson --company "company-uuid"

# Look up user details
raps admin user get "user-abc-123" --account "$ACCOUNT_ID"

# Then add them to projects
raps admin user add-to-all-projects "newhire@company.com" --account "$ACCOUNT_ID"
```
    ...
    steps:
      ...
      - name: Add user to projects
        ...
        run: |
          ...
          raps admin user add "${{ github.event.inputs.email }}" \
            --account "$ACCOUNT_ID" \
...
raps admin user remove "$EMAIL" --account "$ACCOUNT_ID" --dry-run
...
raps admin user remove "$EMAIL" --account "$ACCOUNT_ID" --output json > removal-result.json
...
for email in "${USERS[@]}"; do
  ...
  raps admin user add "$email" --account "$ACCOUNT_ID" --role "$ROLE" \
...
ACCOUNT_ID="your-account-id"
OLD_ROLE="Project Viewer"
NEW_ROLE="Project Admin"
PROJECT_FILTER="name:2024-Active*"

# Get users with old role
raps admin project list --account "$ACCOUNT_ID" --filter "$PROJECT_FILTER" --output json |
...
while read email; do
  ...
  raps admin user update "$email" --account "$ACCOUNT_ID" \
...
|-------|---------|----------|
| `view-only` | View files | Read-only access |
| `view-download` | View + download | Reviewers, clients |
| `upload-only` | Upload only | Submitters |
| `view-download-upload` | View + download + upload | Contributors |
| `view-download-upload-edit` | Full edit access | Team members |
| `folder-control` | Full control + permissions | Administrators |
